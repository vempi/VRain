...
def getpivot(pivoted, dat='',sta='', dirout='', saveas=True):
    ...
    if saveas==True:
        ...
        sns.heatmap(pivoted, cmap='Blues', annot=False, vmax=10, vmin=0,
                    cbar_kws={'label': '[mm/hour]'}, ax=ax)  
        ax.set_ylabel('Duration')

        # Rotate and format x-axis labels
        ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right', fontsize=8)
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))  # Example: "2023-12"
...
def getperiod(dur, dat='',sta='', dirout='', saveas=True):
    ...
    if saveas==True:
        ...
        plt.title(sta)
        plt.savefig(dirout+'VRain_'+dat+'-'+sta+'_histogram-duration.png',dpi=300)
        plt.show()
...
try:root.iconbitmap("2.png")  
except: print("ICO file not found.")
...
input_dir = tk.StringVar(value='E:/OneDrive - UGM 365/Projects-konsultansi/10.PHR-Batch-4/1.Data-internal/Hujan/2.Tabuler-jam-jam-an/PERSIANN_2001-2023_Rantau-bais_merge.csv')
...
output_dir = tk.StringVar(value='E:/OneDrive - UGM 365/Projects-konsultansi/10.PHR-Batch-4/1.Data-internal/Hujan')
