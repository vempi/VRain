...
class FrequencyAnalysisApp:
    ...
    def __init__(self, root):
        ...
        frame_right = tk.Frame(self.root, bg='#f0f0f0')
        ...
        self.plot_container = tk.Frame(frame_right, bg='white')
        self.plot_container.grid(row=0, column=0, sticky="nsew", padx=5, pady=5)
        self.plot_container.grid_propagate(False)
        ...
        self.chk_multiple = tk.Checkbutton(frame_left, text="Enable Batch Analysis (files in a folder)", 
                                          variable=self.check_multiple, command=self.toggle_batch_mode)
        ...
        tk.Label(frame_left, text="Significance Level (α):").grid(row=4, column=0, sticky="w")
        self.alpha = tk.StringVar(value="0.05")
        self.alpha_combobox = ttk.Combobox(frame_left, textvariable=self.alpha, state="readonly", 
                                          values=("0.01", "0.05", "0.10"))
        self.alpha_combobox.grid(row=4, column=1, sticky="w")
        self.alpha_combobox.current(1)
        
        # Return Period Settings
        tk.Label(frame_left, text="Return Periods:").grid(row=5, column=0, columnspan=2, sticky="w")
        self.entry_prob = tk.Entry(frame_left, width=30)
        self.entry_prob.grid(row=6, column=0, columnspan=2, sticky="ew")
        self.entry_prob.insert(0, "1.1,5,10,20,50,100,200")
        
        tk.Button(frame_left, text="Set Probabilities", command=self.convert_values).grid(row=7, column=0, columnspan=2, pady=5)
        
        # Probability Table
        self.tree_prob = ttk.Treeview(frame_left, columns=("Return Period","Probability"), show="headings", height=4)
        self.tree_prob.heading("Return Period", text="Return Period (years)")
        self.tree_prob.heading("Probability", text="Probability")
        self.tree_prob.grid(row=8, column=0, columnspan=2, pady=5, sticky="ew")
        
        # Output Directory
        tk.Label(frame_left, text="Output Directory (optional):").grid(row=9, column=0, sticky="w")
        self.btn_browse_output = tk.Button(frame_left, text="Browse", command=self.browse_output_dir)
        self.btn_browse_output.grid(row=9, column=1, sticky="w")
        
        # Action Buttons
        self.btn_run = tk.Button(frame_left, text="Run Single Column", command=self.perform_anfreq, 
                                bg="#4682B4", fg="white")
        self.btn_run.grid(row=10, column=0, sticky="ew", padx=2, pady=5)
                        
        self.btn_batch_run = tk.Button(frame_left, text="Run All Columns", command=self.batch_run, 
                                      bg="#32CD32", fg="white")
        self.btn_batch_run.grid(row=10, column=1, sticky="ew", padx=2, pady=5)
        ...
        # self.chk_confirm.grid(row=11, column=0, columnspan=2, pady=2, sticky="w")
        
        # Additional Tools
        self.btn_concat = tk.Button(frame_left, text="Concatenate Output CSVs", 
                                   command=self.concatenate_outputs,
                                   state="disabled")  # Start disabled initially
        self.btn_concat.grid(row=11, column=0, columnspan=2, pady=5)
        
        # Save Plot Option
        self.save_plot_var = tk.BooleanVar(value=True)
        self.save_plot_check = tk.Checkbutton(frame_left, text="Save plots to file", 
                                             variable=self.save_plot_var,
                                             command=self.toggle_distribution_selection)
        self.save_plot_check.grid(row=12, column=0, columnspan=2, pady=2, sticky="w")
        
        # ========== DISTRIBUTION SELECTION ========== #
        
        self.dist_frame = tk.LabelFrame(frame_left, text="Select Distributions (Graph)")
        self.dist_frame.grid(row=13, column=0, columnspan=2, pady=5, sticky="ew")
        ...
        col1_frame = tk.Frame(self.dist_frame)
        col1_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        col2_frame = tk.Frame(self.dist_frame)
    ...
    def perform_stat_tests_old(self):
    ...
    def browse_output_dir(self):
        ...
        if folder_selected:
            ...
            self.output_dir = os.path.join(folder_selected)
            if not os.path.exists(self.output_dir):
                os.makedirs(self.output_dir)
    ...
    def batch_run(self):
        ...
        if not self.probabilities_set:
            messagebox.showwarning("Warning", "Please set probabilities first.")
            return
